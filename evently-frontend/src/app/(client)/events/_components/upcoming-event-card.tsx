...
export default function UpcomingEventCard() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const formattedEvents = eventsArray.map((event: any, index: number) => {
        ...
        const startDate = event.startDate || event.dateTime || event.date;
        const endDate = event.endDate || event.end_date;
        let status: "ongoing" | "past" = "ongoing";
        if (startDate && endDate) {
          if (now > new Date(endDate)) status = "past";
          else if (now >= new Date(startDate) && now <= new Date(endDate))
            status = "ongoing";
          else if (now < new Date(startDate)) status = "ongoing";
        } else if (startDate) {
          if (now > new Date(startDate)) status = "past";
          else status = "ongoing";
        }
        return {
      ...
      // Filter on going event jika showOngoing true
      ...
      if (data.pagination) {
        ...
        setTotalEvents(data.pagination.total || filteredEvents.length);
        ...
        setHasNextPage(filteredEvents.length === limit);
    ...
    const testEvents: Event[] = [
      {
        ...
        going: true,
        startDate: new Date(now.getTime() + 3600 * 1000 * 24).toISOString(),
        endDate: new Date(now.getTime() + 3600 * 1000 * 48).toISOString(),
        status: "ongoing",
      },
      {
        id: "test-2",
        ...
        imageUrl: "/placeholder.svg?height=128&width=128",
        going: false,
        startDate: new Date(now.getTime() - 3600 * 1000 * 48).toISOString(),
        endDate: new Date(now.getTime() - 3600 * 1000 * 24).toISOString(),
        status: "past",
      },
    ];
  ...
  }, [page, search, tags, limit, showOngoing]);
        ...
        <div className="space-y-8">
          ...
          <div className="flex flex-wrap gap-4 items-center justify-center">
            ...
            <Button
              ...
              className={showOngoing ? "bg-green-600 text-white" : ""}
              onClick={() => setShowOngoing((prev) => !prev)}
            >
              {showOngoing ? "Show All Events" : "Show Ongoing Events"}
            </Button>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            {events.length === 0 ? (
              ...
              <div className="space-y-8">
                {events.map((event) => (
                  <div key={event.id} className="flex w-full max-w-3xl mx-auto">
                    <div className="w-24 flex flex-col items-start flex-shrink-0">
                    ...
                    <div className="relative flex-grow">
                      ...
                      <Card
                        className="ml-8 bg-neutral-900 border-neutral-700 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors w-full"
                        onClick={() => handleEventClick(event)}
                      >
                        <CardContent className="p-0">
                          <div className="flex p-6 gap-4">
                            <div className="flex-grow">
                              <div className="text-neutral-400 mb-1 text-sm">
                                {event.time}
                              </div>
                                ...
                                <Badge
                                  ...
                                  className={
                                    event.status === "ongoing"
                                      ? "rounded-sm bg-green-600 text-white border-0 px-3"
                                      : "rounded-sm bg-neutral-600 text-white border-0 px-3"
                                  }
                                >
                                  {event.status === "ongoing"
                                    ? "Going"
